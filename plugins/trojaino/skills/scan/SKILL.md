...
1. Use the trusted command prefix supplied by the plugin's SessionStart context. If that context is missing, stop and ask the operator to check setup; do not guess. It contains the **absolute trusted Python 3.11+ path** configured in `TROJAINO_PYTHON` and trusted checkout helper path. Do not guess an interpreter from the candidate directory. The helper is `${CLAUDE_PLUGIN_ROOT}/scripts/preflight.py` in the original trusted checkout, not a copied candidate plugin.
