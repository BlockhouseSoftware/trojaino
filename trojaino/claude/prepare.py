...
Personal-plugin preparation draws its bytes from the installed package, so it
works from a wheel. Whole-checkout preparation still needs a source tree.
"""
...
def _source_payload(personal_plugin_name):
    """Plugin bytes for personal mode; the whole reviewed tree otherwise."""
    if personal_plugin_name is not None:
        from trojaino.claude import payload as packaged

        files = {'plugins/trojaino/' + name: data
                 for name, data in packaged.marketplace_files().items()}
        files['LICENSE'] = (packaged.PAYLOAD / 'LICENSE').read_bytes()
        return files
    builder = ROOT / 'scripts/build_preflight_bundle.py'
    if not builder.is_file():
        raise ValueError('whole-checkout preparation requires a reviewed source tree; '
                         'use --personal-plugin-name to prepare from the installed package')
    import runpy
    import tempfile
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with zipfile.ZipFile(archive_path) as archive:
            return {str(Path(name).relative_to('trojaino-source')).replace('\\', '/'):
...
def _render(destination, personal_plugin_name=None):
    ...
    payload = _source_payload(personal_plugin_name)
...
def prepare(destination, personal_plugin_name=None):
    ...
    from trojaino.claude.writer import write_tree
