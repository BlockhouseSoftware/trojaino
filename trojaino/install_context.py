...
SOURCE_ENV = re.compile(r"^(?:npm_config_(?:registry|userconfig|globalconfig|prefix)|"
                        r"yarn_.*|pnpm_.*|bun_config_.*|"
...
SOURCE_OPTIONS = re.compile(
    r"(?:^|\s)(?:--(?:registry|userconfig|globalconfig|prefix|dir|cwd|directory|config|config-file|"
...
def source_warning(command: str, attempts: list, cwd: str | None) -> str | None:
    ...
    if any(SOURCE_ENV.fullmatch(key) for key in os.environ):
        return "a package source or artifact-selection environment override is active"
